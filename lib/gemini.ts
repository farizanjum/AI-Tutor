...
if (!API_KEY) {
  // Fail fast and avoid using an embedded fallback key which will cause
  // confusing 400 errors from the API. The user must set GEMINI_API_KEY.
...
CRITICAL: You MUST ALWAYS respond with valid JSON in the exact format below. No exceptions, no plain text responses, no markdown formatting. Every response must be parseable JSON starting with { and ending with }.
...
MANDATORY RESPONSE FORMAT: You MUST always respond with valid JSON in this exact structure. No plain text responses allowed.

{
  "diagnosis": "<one-line summary of level & key gap(s), or 'General inquiry' for non-tutorial questions>",
  "plan": [
  ...
  "explanations": {
    ...
    "worked_example":"step-by-step explanation or main content",
    "deep":"optional deeper explanation or additional details"
  },
  "safety_notice":"<if applicable>"
}

Optimization & parameters:
• When users request detailed explanations (like "explain in detail", "at least 500 words"), provide comprehensive, in-depth responses in the "deep" field
• Maximum response length: 2000 tokens (sufficient for detailed explanations)
...
• ALWAYS return valid JSON - this is non-negotiable.
• For detailed topics, break down complex subjects into multiple steps with comprehensive explanations.`
...
export async function callGemini(
  ...
  try {
    console.log("[v0] Starting Gemini API call with model: gemini-1.5-flash")
    ...
    // If the SDK provided a fetch error with status, surface it clearly.
    // The SDK throws GoogleGenerativeAIFetchError with properties: status, statusText, errorDetails
    // We can't reliably import the SDK error class here, so check for properties.
    if (error && typeof error === "object") {
      ...
      if (status) {
        console.error(`[v0] Gemini HTTP error: ${status} ${statusText || ""}`)
        if (details) console.error("[v0] Error details:", details)
      }
    }
